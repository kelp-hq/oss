<script lang="ts">
	...
	$: {
		if (
			!isNil($polkadotAccountsStore.selectedAccount) &&
			!isEmpty($polkadotAccountsStore.selectedAccount)
		) {
	...
	<div class="flex items-center gap-2">
		...
		<label tabindex="0" class="btn btn-ghost btn-circle avatar">
			<img
				class="rounded-full"
				alt="Identicon"
				src="data:image/png;base64,{avatarIconImageBase64String}"
			/>
		</label>
	</div>

	<div
		class="flex flex-col dropdown-content bg-base-300 p-4 shadow-lg drop-shadow-2xl w-96 rounded-lg"
	>
