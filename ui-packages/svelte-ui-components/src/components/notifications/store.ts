...
function notificationsFn() {
  ...
  return {
    ...
    addNew: (options: {
      ...
      const { text, autoclose: origAutoclose, infoLevel, cssClass, closeCb, showSpinner } = options;
      ...
      let autoClose: IAutoClose = origAutoclose as IAutoClose;
      if (isNil(autoClose)) {
      ...
      const a: INotification = {
        ...
        text,
        autoclose: autoClose,
        infoLevel: infoLevel as InfoLevel,
        closeFn,
        cssClass: cssClass as string,
        showSpinner: showSpinner as boolean
      };
    ...
    add: (
      ...
      autoclose: IAutoClose = { close: true, time: 3000 }
    ) => {
      const id = nanoid();
      const a: INotification = {
        id,
...
export const notificationsStore = notificationsFn();
