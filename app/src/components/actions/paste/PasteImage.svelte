  ...
  document.addEventListener('paste', (event) => {
    ...
    for (const index in items) {
      ...
      if (item.kind === 'file') {
        ...
        console.log('Got paste event with DataTransferItem kind %s and type %s', item.kind, item.type);
      }
    }
  });
});
    ...
    <div class="flex flex-row gap-6">
      <div class="w-2/3  min-h-fit">
        <div class="h-full rounded-md">
          ...
          <!-- {#if $pasteImageStore.src && !loadingBlob}
					...
					{/if} -->
          <canvas bind:this="{canvasElement}" class="bg-base-300 w-full"></canvas>
        </div>
      </div>
      <!-- SIDE BAR -->
      <div class="w-1/3 flex flex-col gap-4 items-center">
        <div class="w-full flex justify-between">
          ...
          <button class="btn btn-ghost btn-outline normal-case btn-secondary" on:click="{convertImage}"
            >Show metadata</button
          >
          <button class="btn btn-primary" disabled="{isEmpty($pasteImageStore.src)}" on:click="{uploadImage}"
            >Upload</button
          >
        </div>
        <div class="w-full flex flex-col gap-4 items-center form-control">
          <textarea
            ...
            bind:value="{$pasteImageStore.description}"></textarea>
