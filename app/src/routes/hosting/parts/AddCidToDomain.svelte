<script lang="ts">
	...
	let websiteCid: string = 'bafybeigeurtsjr7scmhufyy5zxxgneexmow7rswfoucofyarhxtt6em4ba';
	...
	async function makeToken() {
		...
		sig = await signViaExtension($polkadotAccountsStore.selectedAccount, encodedPayload);
...
<div class="m-4 w-96 bg-primary text-primary-content shadow-xl rounded-md p-2">
	...
	<div class="form-control p-4">
		<div class="input-group ">
			...
			<button
				on:click={addCid}
				class="btn  w-24 {sendingRequest ? 'loading' : ''} {!!websiteCid ? '' : 'btn-disabled'}"
				>ADD</button
			>
		</div>
	</div>
